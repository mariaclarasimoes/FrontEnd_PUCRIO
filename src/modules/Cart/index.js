...
const Cart = () => {
  ...
  if(carts.length === 0) {
    return <div className=' h-[55vh] flex justify-center items-center text-4xl uppercase'>Carrinho vazio</div>
  }
      ...
      <div className="flex shadow-md my-10">
        <div className="w-3/4 bg-white px-10 py-10">
          <div className="flex justify-between border-b pb-8">
            <h1 className="font-semibold text-2xl">MEU CARRINHO DE COMPRAS</h1>
            <h2 className="font-semibold text-2xl">{carts?.length} Itens</h2>
          </div>
                ...
                <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                  <div className="flex w-2/5">
                    ...
                    <div className="flex flex-col justify-between ml-4 flex-grow">
                      ...
                      <div className="font-semibold hover:text-red-500 text-gray-500 text-xs cursor-pointer" onClick={() => removeProduct(cart?.id)}>Remover produto</div>
                    </div>
                  </div>
                  ...
                  <span className="text-center w-1/5 font-semibold text-sm">R$ {cart?.price}</span>
                  <span className="text-center w-1/5 font-semibold text-sm">R$ {cart?.price * cart?.quantity}</span>
                </div>
              )
            })
          }
        ...
        <div id="summary" className="w-1/4 px-8 py-10">
          <h1 className="font-semibold text-2xl border-b pb-8 uppercase">Resumo do Pedido</h1>
          <div className="flex justify-between mt-10 mb-5">
            <span className="font-semibold text-sm uppercase">{carts?.length} Itens </span>
            <span className="font-semibold text-sm">R$ {total?.toFixed(2)}</span>
          </div>
          <div>
            <label className="font-medium inline-block mb-3 text-sm uppercase">Frete</label>
            <select className="block p-2 text-gray-600 w-full text-sm">
              <option>Entrega própria - R$ 19.99</option>
              <option>Correios - R$ 17.50</option>
            </select>
          </div>
          <div className="py-10">
            <label for="promo" className="font-semibold inline-block mb-3 text-sm uppercase">Código promocional</label>
            <input type="text" id="promo" placeholder="Digite o código" className="p-2 text-sm w-full" />
          </div>
